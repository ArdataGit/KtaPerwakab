...
<x-layouts.mobile title="Edit Produk">
    ...
    {{-- ==================== DESKTOP VIEW ==================== --}}
    <x-slot:desktop>
        <x-desktop.layout title="Edit Produk">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 space-y-6">
                    ...
                    @if(!empty($existing_photos))
                        <div>
                            ...
                            <div class="flex gap-3 flex-wrap">
                                @foreach($existing_photos as $photo)
                                    <div class="relative">
                                        <img src="{{ api_product_url($photo['file_path']) }}" class="w-24 h-24 rounded-xl object-cover border border-gray-200 shadow-sm">
                                        <button type="button" onclick="if(confirm('Yakin ingin menghapus foto ini?')) { @this.call('deletePhoto', {{ $photo['id'] }}) }"
                                            class="absolute -top-2 -right-2 bg-red-500 hover:bg-red-600 text-white rounded-full w-7 h-7 flex items-center justify-center text-base font-bold shadow-lg transition">×</button>
                                    </div>
                                @endforeach
                            </div>
                            <p class="text-xs text-gray-500 mt-2">Klik tombol × untuk menghapus foto</p>
                        </div>
                    @endif
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div class="md:col-span-2">
                            <label class="block text-sm font-medium text-gray-700 mb-1">Nama Produk <span class="text-red-500">*</span></label>
                            <input type="text" wire:model.live="product_name" placeholder="Masukkan nama produk" class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-green-500 outline-none text-sm bg-gray-50 transition">
                        ...
                        <div class="md:col-span-2">
                            <label class="block text-sm font-medium text-gray-700 mb-1">Deskripsi (Opsional)</label>
                            <textarea wire:model="description" rows="4" placeholder="Deskripsi produk..." class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-green-500 outline-none text-sm bg-gray-50 resize-none transition"></textarea>
                        </div>
                        <div class="md:col-span-2">
                            <label class="block text-sm font-medium text-gray-700 mb-1">Link YouTube (Opsional)</label>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <p class="text-xs text-gray-500 mt-2">Kosongkan jika tidak ingin menambah foto baru</p>
                        </div>
                    </div>
                    <button wire:click="submit" class="w-full bg-green-600 hover:bg-green-700 text-white py-4 rounded-xl font-semibold text-base transition shadow-md shadow-green-200">Update Produk</button>
                </div>
            </div>
        </x-desktop.layout>
    </x-slot:desktop>
